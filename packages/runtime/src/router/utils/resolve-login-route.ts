...
const isLoginRoute = (r: AppRouteRecordRaw) => r.handle?.login === true;
const isInternalLoginRoute = (r: any) => r?.handle?.login === true && r?.handle?.internal === true;
...
export function resolveLoginRoute(
	...
	incoming: AppRouteRecordRaw[],
	router: RouterLike,
	rootRoute: RouteObject[],
): AppRouteRecordRaw[] {
	let winner: AppRouteRecordRaw | undefined;
	for (const route of incoming) {
		if (!isLoginRoute(route) || route.handle?.internal === true) {
			continue;
		}
	...
	router.patchRoutes(ROOT_ROUTE_ID, [winner as never]);
