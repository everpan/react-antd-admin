/* data-ram-runtime-css: 产物自携带样式（偏差 3 修复），由 scripts/inline-css.mjs 注入 */
...
*/
function isLoginPathname(pathname, base = "/") {
	return pathname === `${base}${loginPath.slice(1)}`;
}
var loginPath, exceptionPath, exception403Path, exception404Path, exception500Path;
...
var AUTH_HEADER, LANG_HEADER, REFRESH_TOKEN_PATH, AUTH_LOGIN_PATH;
...
//#endregion
//#region src/assets/svg/banner.svg?inline
var banner_default;
...
//#endregion
//#region src/layout/fullscreen-layout/index.tsx
/**
...
var init_fullscreen_layout = __esmMin((() => {
	init_banner();
	init_logo$2();
...
//#endregion
//#region src/utils/get-redirect-path.ts
/**
...
var init_get_redirect_path = __esmMin((() => {}));
//#endregion
...
function Login$1() {
	...
	return /* @__PURE__ */ jsx(FormModeContext, {
		value: providedValue,
...
var init_auth$1 = __esmMin((() => {
	...
	routes$1 = [{
		...
		children: [{
			index: true,
			Component: Login,
			handle: { title: $t("authority.login") }
		}]
	}];
}));
//#endregion
	...
	*/
	if (pathname === "/login") {
		...
		*/
		...
		if (redirectPath !== pathname) return /* @__PURE__ */ jsx(Navigate, {
			to: redirectPath,
			replace: true
		});
		return /* @__PURE__ */ jsx(Navigate, {
...
//#endregion
//#region src/router/utils/resolve-login-route.ts
/**
...
*/
function resolveLoginRoute(base, incoming, router, rootRoute) {
	let winner;
	for (const route of incoming) {
		if (!isLoginRoute(route) || route.handle?.internal === true) continue;
		if (route.path !== "/login") {
			console.warn(`[login] handle.login 拒绝：契约路径必须是 ${loginPath}，收到 "${route.path}"`);
			continue;
		}
		if (winner) {
			console.warn(`[login] 重复的 login 路由忽略：${loginPath} 已由先注册的模块提供`);
			continue;
		}
		winner = route;
	}
	...
	router._internalSetRoutes([{
		...root,
		children: (root.children ?? []).filter((r) => !isInternalLoginRoute(r))
	}]);
	router.patchRoutes(ROOT_ROUTE_ID, [winner]);
	return base.filter((r) => !isInternalLoginRoute(r));
}
var isLoginRoute, isInternalLoginRoute;
var init_resolve_login_route = __esmMin((() => {
	init_constants();
	init_extra_info();
	isLoginRoute = (r) => r.handle?.login === true;
	isInternalLoginRoute = (r) => r?.handle?.login === true && r?.handle?.internal === true;
}));
//#endregion
...
var init_access = __esmMin((() => {
	...
	useAccessStore = create((set) => ({
		...
		setAccessStore: (routes) => {
			const newRoutes = ascending([...resolveLoginRoute(baseRoutes, routes, router, rootRoute), ...routes]);
...
//#endregion
//#region src/utils/request/whitelist.ts
function isAnonymousApi(url) {
	return anonymousApiPaths.some((path) => url.endsWith(path));
}
var anonymousApiPaths;
var init_whitelist = __esmMin((() => {
	init_constants$4();
	anonymousApiPaths = [AUTH_LOGIN_PATH];
}));
//#endregion
//#region src/utils/request/index.ts
var defaultConfig, request;
			...
			afterResponse: [async ({ request, options, response }) => {
				...
				if (!response.ok) {
					if (response.status === 401) {
						...
						if (!refreshToken) {
							if (isLoginPathname(location.pathname)) return response;
							else {
								goLogin();
								return response;
							}
